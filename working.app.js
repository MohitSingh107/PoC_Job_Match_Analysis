...
const renderImprovedHeader = (headerData) => {
  ...
  return (
    <div style={{ marginBottom: '1.2rem', textAlign: 'center' }}>
      ...
      {/* Extract title from other array */}
      {(() => {
        const title = headerData.other.find(item => 
          item && 
          !item.includes('@') && 
          !item.match(/\d{10}/) && 
          !item.toLowerCase().includes('linkedin') && 
          !item.toLowerCase().includes('github') &&
          !item.toLowerCase().includes('kaggle') &&
          !item.match(/https?:\/\//i) &&
      ...
      {/* Contact Info - Single Horizontal Row with Separators */}
      {(() => {
        const contactItems = [];
        
        if (headerData.location) {
          contactItems.push(
            <span key="location">{headerData.location}</span>
          );
        }
            ...
            {contactItems.map((item, idx) => (
              <React.Fragment key={item.key || idx}>
                ...
                {idx < contactItems.length - 1 && (
                  <span style={{ color: '#9ca3af', margin: '0 0.15rem', flexShrink: 0 }}>|</span>
                )}
              </React.Fragment>
            ))}
          </div>
...
const ResumeComparison = ({ analysisData, fileUrl, fileType, formatResumeText }) => {
   ...
   */
    ...
    lines.forEach((line, idx) => {
      ...
      const isCertificationEntry = sectionType === 'certifications' && 
                                    !isBullet && 
                                    !isCertificateLinkLine &&
                                    !trimmed.toLowerCase().startsWith('certificate link:') &&
                                    (hasPipeSeparators || (trimmed.length > 5 && trimmed.length < 100 && !trimmed.includes(':')));
      ...
      switch (item.type) {
        ...
        case 'project-title':
          ...
          if (projectParts.length > 0) {
            projectName = projectParts[0].replace(/\[CN Project\]/g, '').trim();
            if (projectParts.length > 1) {
              projectTech = projectParts[1];
            }
          } else {
        ...
        case 'certification-entry':
          ...
          if (certParts.length >= 3) {
            // Format: "Title | Organization | Year"
  ...
  const renderOriginalWithDiff = () => {
    ...
    // Define section order: projects should come before education
    const sectionOrder = {
      'header': 1,
      'summary': 2,
      'skills': 3,
    ...
    // Sort sections to ensure projects come before education
    const sortedSections = [...sections].sort((a, b) => {
      const orderA = sectionOrder[a.type] || 99;
      const orderB = sectionOrder[b.type] || 99;
  ...
  const renderImprovedWithDiff = () => {
    ...
    // Sort sections to ensure projects come before education
    const sortedSections = [...sectionsWithContent].sort((a, b) => {
      const orderA = sectionOrder[a.type] || 99;
      const orderB = sectionOrder[b.type] || 99;
      return orderA - orderB;
    });
  ...
  const parseResumeSectionsSimple = (text, sectionsInfo = null) => {
    ...
    // Helper to check if a line looks like header content (name, contact info) vs a section title
    const isHeaderContent = (line, headerLinesCount) => {
      const trimmed = line.trim();
      // If we haven't captured header yet and this looks like a name or contact info
      if (!headerCaptured && headerLinesCount === 0) {
        // Name-like: 2-4 words, all caps, no section keywords
        const words = trimmed.split(/\s+/);
        if (words.length >= 2 && words.length <= 4 && 
            trimmed === trimmed.toUpperCase() &&
            !trimmed.match(/(SUMMARY|SKILLS|EXPERIENCE|EDUCATION|PROJECTS|CERTIFICATIONS|AWARDS|LANGUAGES)/i)) {
          return true;
        }
      }
      // Contact info: email, phone, location, URLs
      if (trimmed.includes('@') || 
          trimmed.match(/\d{10}/) || 
          trimmed.match(/linkedin|github|kaggle/i) ||
    ...
    lines.forEach((line) => {
      ...
      // If it matches 'other' pattern but looks like header content, treat as header content
      if (sectionType === 'other' && isHeaderContent(trimmed, headerLines.length)) {
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '2rem 1rem' }}>
          <header style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
              ...
              {/* Download button temporarily disabled per request */}
              {/*
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <button
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 1rem 2rem' }}>
          ...
          <footer style={{
            ...
            <div style={{
              ...
              Your privacy is our priority. All analysis is confidential. © 2025 Coding Ninjas. All rights reserved.
            </div>
          </footer>
        </div>
    );
